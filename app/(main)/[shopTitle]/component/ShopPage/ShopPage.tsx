...
import { observer } from "mobx-react-lite";
 ...
 const ShopPage = observer(({shopData} : any) => {
  ...
  return (
    <Box>
      <Box position="sticky" top="0" zIndex="50" w="full" bg="white" boxShadow="sm" borderBottom="1px" borderColor="gray.200" display={{ base: "block", md: "none" }}>
        <Flex h="14" alignItems="center" px="4">
          <Flex flex="1" justify="center">
            <Heading as="h1" size="md" fontWeight="semibold">{shopData.name}</Heading>
          </Flex>
          <Badge colorScheme={isOpen24Hours() ? "green" : "gray"} variant={isOpen24Hours() ? "solid" : "outline"}>{isOpen24Hours() ? "Open Now" : "Closed"}</Badge>
        </Flex>
      </Box>
      ...
      <Container maxW="container.xl" px="4" py="8" id="about">
        <Grid templateColumns={{ base: "1fr", md: "2fr 1fr" }} gap="8">
          <GridItem><ShopAbout shopData={shopData} /></GridItem>
          <GridItem><OperatingHours shopData={shopData} getCurrentDayHours={getCurrentDayHours} /></GridItem>
        </Grid>
      </Container>
      <Box id="gallery"><ShopImages shopData={shopData} /></Box>
      <Container maxW="container.xl" id="products">
        <CommonHeading heading="Our Products" subheading="Discover Our most popular products" mb={{ base: 4, md: 8 }} />
        <Grid templateColumns={{ base: "repeat(1, 1fr)", md: "repeat(3, 1fr)", lg: "repeat(4, 1fr)" }} gap={{ base: 4, md: 3, lg: 4 }} justifyItems="center">
